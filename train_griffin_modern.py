...
def diagonal_linear_scan(decay: Tensor, inputs: Tensor, initial_state: Tensor | None, chunk_size: int) -> tuple[Tensor, Tensor]:
    ...
    for start in range(0, seq_len, chunk):
        ...
        chunk_states = torch.empty_like(b)
        for offset in range(end - start):
            state = a[:, offset, :] * state + b[:, offset, :]
...
def build_optimizer(model: HybridGriffinLanguageModel, config: TrainConfig) -> torch.optim.Optimizer:
...
def save_checkpoint(path: str | Path, model: HybridGriffinLanguageModel, optimizer: torch.optim.Optimizer | None, config: ExperimentConfig, step: int, extra: dict[str, Any] | None = None) -> None:
    path = Path(path)
...
def load_checkpoint(path: str | Path, model: HybridGriffinLanguageModel, optimizer: torch.optim.Optimizer | None = None, map_location: str | torch.device = "cpu") -> dict[str, Any]:
...
def run_training_loop(model: HybridGriffinLanguageModel, train_loader: StreamingTokenLoader, val_tokens: Tensor, config: ExperimentConfig, device: torch.device, amp_dtype: torch.dtype, tokenizer_luts: tuple[Tensor, Tensor, Tensor] | None) -> None:
    ...
    for step in range(start_step, config.train.max_steps):
        ...
        if step % config.train.log_interval == 0 or step == config.train.max_steps - 1:
            ...
            extra["tok_s"] = float(batch["targets"].numel()) / max(avg_step_time, 1e-9)
            if device.type == "cuda":
    ...
    if config.train.legacy_export_path:
        print(f"legacy_export_skipped:{config.train.legacy_export_path} reason:standalone_train_griffin_no_exporter")
...
def parse_args() -> ExperimentConfig:
    ...
    parser.add_argument("--mix-strategy", choices=["learned", "sum", "recurrent_only", "attention_only"], default="learned")
